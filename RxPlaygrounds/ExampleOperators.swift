...
struct ExampleOperators {
    
    let delayOperator = Operator(input: [
        SampleInput(value: .number(1), timeOffset: 10),
        SampleInput(value: .number(2), timeOffset: 20),
        SampleInput(value: .number(1), timeOffset: 30)
        ], parameterInput: nil, apply: { input, _ in
            return input.delay(0.1, scheduler: MainScheduler.instance)
        }, summary: "shift the emissions from an Observable forward in time by a particular amount", formula: "delay", detailDocument: "The Delay operator modifies its source Observable by pausing for a particular increment of time (that you specify) before emitting each of the source Observable’s items. This has the effect of shifting the entire sequence of items emitted by the Observable forward in time by that specified increment.")
    
    let mapOperator = Operator(input: [
        SampleInput(value: .number(1), timeOffset: 20),
        SampleInput(value: .number(2), timeOffset: 40),
        SampleInput(value: .number(3), timeOffset: 70)
        ], parameterInput: nil, apply: { input, _ in
            return input.map({
                if case let .number(x) = $0 {
                    return SampleValue.number(x * 10)
                }
                return $0
            })
        }, summary: "transform the items emitted by an Observable by applying a function to each item", formula: "map({ $0 * 10 })", detailDocument: "The Map operator applies a function of your choosing to each item emitted by the source Observable, and returns an Observable that emits the results of these function applications.")
    
    let combineLatestOperator = Operator(input: [
        SampleInput(value: .number(1), timeOffset: 10),
